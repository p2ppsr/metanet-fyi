...
function recovery () {
  ...
  return `<article class="kit-page"><header class="kit-hero"><div class="eyebrow"><span>Overlay field kit</span><span>Recovery</span><span>7 checks</span></div><h1>Will your data<br><em>outlive your app?</em></h1><p class="hero-deck">“On-chain” answers where a commitment was made. This test asks the harder question: could another person still retrieve, interpret, validate, and serve the record if you disappeared?</p><div class="score-card"><span>Your working score</span><strong><output id="recovery-score">0</output><small>/ 7</small></strong><p>No data leaves this page. This checklist is stored only in your browser.</p></div></header>
  <section class="checklist-section"><div class="checklist-intro"><p class="section-number">The survival test</p><h2>Seven “yes” answers<br>make a <em>credible path.</em></h2><p>This is an operational test, not a badge. A resilient system periodically proves these claims from a clean machine.</p><button class="reset-button" type="button" id="reset-checks">Reset checks</button></div><ol class="survival-checklist">${checks.map(([number, title, question, action]) => `<li><input type="checkbox" id="check-${number}" data-recovery-check><label for="check-${number}"><span>${number}</span><span><b>${title}</b><strong>${question}</strong><small>${action}</small></span><i aria-hidden="true">✓</i></label></li>`).join('')}</ol></section>
